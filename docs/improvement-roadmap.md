...
#### MD-A03 生命周期/灭屏解锁恢复与能力上报

- **范围**：处理 Activity 的 `onPause`、`onStop`、`onResume`、灭屏/亮屏和 WebView 进程异常后的恢复；重新建立页面、SSE 和状态同步；上报全屏、常亮、WebView 连接、Lock Task、Device Owner/DPC 等实际能力与失败原因。普通模式先解决用户解锁后的恢复，不以后台服务冒充前台恢复。
- **产物**：生命周期恢复状态机、能力诊断/上报字段和界面、普通模式恢复测试记录、异常状态提示。
- **验收标准**：普通模式在用户完成解锁后 1 秒内恢复可见投放画面；连续 20 次灭屏/亮屏循环均能恢复；断网重连和服务端重启后能重新同步；横竖屏切换及不同分辨率下画面不丢失；任何能力不可用或恢复失败都显示原因，不静默宣称已锁定。
- **依赖**：MD-A02；需要可重复控制灭屏/亮屏、断网和服务端重启的测试环境。
