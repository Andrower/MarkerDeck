...
状态：已选定四角取景框与中心方点方案，Android 自适应彩色图标、主题单色图标和标准 SVG 源稿已实现，Lint 与 debug APK 构建通过，待真机 launcher 验收。

- **设计**：使用深墨绿背景、米白取景框四角和亮绿中心方点，避免旧蓝色十字产生添加或医疗符号联想。
- **适配**：前景保持在 Android 自适应图标安全区域，单色资源保留相同剪影供系统主题着色。
- **边界**：只更新 Android launcher 图标；应用页面、协议、包名、版本号和 Electron 打包配置不变。
- **依赖**：Android 资源编译与至少一台真机 launcher 视觉验收，详细记录见 `docs/tasks/MD-A19.md`。
