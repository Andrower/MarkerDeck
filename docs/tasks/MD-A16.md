...
基于 `d3a7602` 完成单手机宿主与真实浏览器控制闭环、20 个模拟 SSE 接收会话、Wi-Fi/USB 对照和短时后台检查。追加使用 Node `http.request`、每次 POST 新建连接的对照后，20 个模拟会话收到 `4000/4000` 个事件，delivery P50 `29.32ms`、P95 `47.23ms`、最大 `68.75ms`，POST P95 `43.75ms`；这不是 20 台实机容量认证。连接复用的合成测试仍出现约半秒 P95 延迟波动，客户端/连接交互原因待继续定位，不能据此归因于手机算力或普通控制阻塞。v1.7.0 APK 已在 `22127RK46C` 修复并验收宿主控制页状态栏安全区，local/display 沉浸式投放保持不变。方法、完整数值和未测边界见 [手机宿主性能记录](../verification/android-host-performance-2026-09-05.md)。
