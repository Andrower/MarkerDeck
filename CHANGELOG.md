...
- 完成 MD-A17 控制端场景工作台：桌面/平板提供顶栏快速连接与场景管理、按分组选择设备和等比例场景平面图；场景尺寸与接收页面归一化坐标按版本保存到浏览器 localStorage。设备分组支持全选、部分选中、多会话展开/收起，场景拖拽期间设备刷新不再替换卡片；手机隐藏完整平面图编辑与重复预设网格，保留设备选择、批量锁定和底部预设入口；local/display 继续保留本地快速调节、快捷预设、锁定/退出和同步流程，不展示场景控制台。补充 Node/Android `markerdeck-scenes.js` 静态资源白名单与专项测试；Android 本机宿主控制页已完成真机横竖屏检查，其他 OEM 未测。
- Android 本机作为宿主的控制页补充状态栏与刘海安全区顶部 inset；该适配只作用于 `control` WebView，`local`/`display` 投放继续使用沉浸式窗口和原有锁定语义，不改变 HTTP、SSE、ACK 或导出协议。
